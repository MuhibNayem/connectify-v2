...
func (r *GraphRepository) CheckFriendshipStatus(ctx context.Context, me, other primitive.ObjectID) (bool, bool, bool, bool, bool, error) {
	query := `
	...
	if len(result.Records) == 0 {
		// Try syncing users if they don't exist?
		// For now fail safe
		return false, false, false, false, false, nil
	}
	rec := result.Records[0]
	// Handle nil values if nodes don't exist, though MATCH should filter them out if not found?
