...
func (s *AuthService) Register(ctx context.Context, user *models.User) (*models.AuthResponse, error) {
	...
	// Async sync to Neo4j
	go func() {
		if err := s.graphRepo.SyncUser(context.Background(), createdUser.ID); err != nil {
			log.Printf("Failed to sync user to graph: %v", err)
		}
	}()
...
func (s *AuthService) Logout(ctx context.Context, userID, accessToken string) error {
	...
	s.redisClient.Set(ctx, "blacklist:"+accessToken, "1", s.cfg.AccessTokenTTL)
...
func (s *AuthService) generateTokens(ctx context.Context, user *models.User) (string, string, error) {
	...
	accessToken, _ := jwt.NewWithClaims(jwt.SigningMethodHS256, accessClaims).SignedString([]byte(s.cfg.JWTSecret))
	...
	refreshToken, _ := jwt.NewWithClaims(jwt.SigningMethodHS256, refreshClaims).SignedString([]byte(s.cfg.JWTSecret))

	err := s.redisClient.Set(ctx, "refresh:"+user.ID.Hex(), refreshToken, s.cfg.RefreshTokenTTL).Err()
